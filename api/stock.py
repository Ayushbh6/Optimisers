"""Stateless Vercel adapter for the Stock Watch showcase."""
...
def _restored(case_id: str, issue_ids: list[str]) -> tuple[StockWatchService, dict]:
    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if route == "/api/examples":
                self._json(HTTPStatus.OK, StockWatchService().examples())
                return
            if route.endswith("/export-order") or route.endswith("/export-risk"):
                ...
                if route.endswith("/export-order"):
                    body = service.export_order(session["session_id"]).encode()
                    filename = "stock-watch-order.csv"
                else:
                    body = service.export_risk(session["session_id"]).encode()
                    filename = "stock-watch-risk-list.csv"
                self._headers(HTTPStatus.OK, "text/csv; charset=utf-8", len(body), f'attachment; filename="{filename}"')
                self.wfile.write(body)
                return
